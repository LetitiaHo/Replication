...
rm -r R-hacks
```
Change the address of R-hacks listed in a text file to enable access to R-hacks
```
emacs /home/ubuntu/modelblocks-release/config/user-rhacks-directory.txt
```
Change the address listed in the file from `/home/compling/R-hacks` to `/home/ubuntu/modelblocks-release/resource-rhacks`. Then press `ctrl-x` then `ctrl-c` to save and exit the file.
