...
Create an Amazon EC2 server by creating an [AWS](https://aws.amazon.com/) account. You will be creating a server that will provide you with access to GPUs, which will be required to run the neural networks training and testing in seconds instead of hours or days.
...
```
### 3.2 Analysis 2: Adapting the model to other story genres.
...
rm -r R-hacks
```
Change the location of R-hacks listed `user-rhacks-directory.txt` to enable access to R-hacks
```
emacs /home/ubuntu/modelblocks-release/config/user-rhacks-directory.txt
```
Change the location within the file from `/home/compling/R-hacks` to `/home/ubuntu/modelblocks-release/resource-rhacks`. Then press `ctrl-x` then `ctrl-c` to save and exit the file.
### 4.2 Edit the column label for noadapt results
...
```
### 4.3 Compiling adapt and noadapt results
Past the `surpnoa` column from each `noadapt.results` file to the corresponding `adapt.results` file.
```
...
### 4.5 Manually generating needed files
May need python2 instead of python3??
```
